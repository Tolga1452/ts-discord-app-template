...
let pkg = { name: "component", version: "1.0.0", bin: { component: "scripts/component.js" } };
try {
...
function commandFilePath(name) {
  return resolve(projectRoot, "src", "interactions", "components", `${name}.ts`);
}
...
function main() {
  ...
  switch (command) {
    case "create": {
      const name = args[1] ?? flags.name ?? "new-component";
      const content = `import { ComponentType } from 'discord.js';
...
export default {
    ...
    async execute(interaction, args) {
        ...
        await interaction.update2('Button clicked!');
    }
...
`;
      ...
      try {
        ...
        log.ok(`Created ${c.bold(`src/interactions/components/${name}.ts`)}`);
      } catch (e) {
    ...
    case "remove": {
      ...
      if (!name) {
        log.err("No component name specified.");
        process.exitCode = 1;
        return;
      }
      const filePath = commandFilePath(name);
      if (!existsSync(filePath)) {
        log.warn(`No such component file: ${c.bold(`src/interactions/components/${name}.ts`)}`);
      } else {
        try {
          unlinkSync(filePath);
          log.ok(`Removed ${c.bold(`src/interactions/components/${name}.ts`)}`);
        } catch (e) {
    ...
    case "clear": {
      const dir = resolve(projectRoot, "src", "interactions", "components");
      if (!existsSync(dir)) {
        log.warn('No such directory: ' + c.bold('src/interactions/components'));
      } else {
        ...
        for (const f of files) {
          ...
          try {
            unlinkSync(fp);
            log.ok('Removed ' + c.bold('src/interactions/components/' + f));
          } catch (e) {
