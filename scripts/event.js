...
let pkg = { name: "event", version: "1.0.0", bin: { event: "scripts/event.js" } };
try {
...
function commandFilePath(name) {
  return resolve(projectRoot, "src", "events", `${name}.ts`);
}
...
function printHelp() {
  ...
  const examples = `${c.bold("Examples")}\n  ${BIN} ${c.cyan("create")} ready\n  ${BIN} ${c.cyan("remove")} ready\n  ${BIN} ${c.cyan("clear")}`;
...
function main() {
  ...
  switch (command) {
    case "create": {
      const name = args[1] ?? flags.name ?? "newEvent";
      const content = `import { GatewayEvent } from '../types/index.js';

export default {
    name: '${name}',
    async execute() {
    }
} satisfies GatewayEvent<'${name}'>;
`;
      const filePath = commandFilePath(name);
      ensureDirFor(filePath);
      try {
        writeFileSync(filePath, content, "utf8");
        log.ok(`Created ${c.bold(`src/events/${name}.ts`)}`);
      } catch (e) {
        log.err(`Failed to write file: ${filePath}`);
        process.exitCode = 1;
        return;
      }
      run("pnpm update-events");
    ...
    case "remove": {
      ...
      if (!name) {
        log.err("No event name specified.");
        process.exitCode = 1;
        return;
      }
      const filePath = commandFilePath(name);
      if (!existsSync(filePath)) {
        log.warn(`No such event file: ${c.bold(`src/events/${name}.ts`)}`);
      } else {
        try {
          unlinkSync(filePath);
          log.ok(`Removed ${c.bold(`src/events/${name}.ts`)}`);
        } catch (e) {
    ...
    case "clear": {
      const dir = resolve(projectRoot, "src", "events");
      if (!existsSync(dir)) {
        log.warn('No such directory: ' + c.bold('src/events'));
      } else {
        ...
        for (const f of files) {
          ...
          try {
            unlinkSync(fp);
            log.ok('Removed ' + c.bold('src/events/' + f));
          } catch (e) {
